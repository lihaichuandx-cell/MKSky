...
// 还原历史的分块机制：死板的静态块，产生大量冗余的局部候选人
static __global__ void hyb_local_baseline_kernel(int n, int dim, const float* d_coords, int* d_local_survivors) {
	...
	if (i < n) {
		...
		d_local_survivors[i] = is_local_skyline ? 1 : 0;
	}
}

static __global__ void hyb_extract_candidates_kernel(int n, const int* d_local_survivors, int* d_candidate_indices, int* d_candidate_count) {
	int i = blockIdx.x * blockDim.x + threadIdx.x;
...
// 全局合并灾难：不利用树状归并，强行 O(C^2) 互相伤害
static __global__ void hyb_global_brute_kernel(int dim, int chunk_start, int chunk_size, int num_candidates, const float* d_coords, const int* d_candidates, int* d_final_skyline, int n) {
...
std::tuple<int, double> run_hybrid_v2(std::vector<MyDataPoint>& h_points, const AlgorithmConfig& config) {
	...
	hyb_local_baseline_kernel << <grid_all, block >> > (n, dim, d_coords, d_local_survivors);
	hyb_extract_candidates_kernel << <grid_all, block >> > (n, d_local_survivors, d_candidate_indices, d_candidate_count);

	int num_candidates = 0;
	CHECK_CUDA_ERROR(cudaMemcpy(&num_candidates, d_candidate_count, sizeof(int), cudaMemcpyDeviceToHost));

	// 切片只是为了在你的笔记本上不报错退出，不改变其庞大的计算量
	int chunk_size = 4096;
	for (int start = 0; start < num_candidates; start += chunk_size) {
		...
		hyb_global_brute_kernel << <chunk_grid, block >> > (dim, start, size, num_candidates, d_coords, d_candidate_indices, d_final_skyline, n);
		CHECK_CUDA_ERROR(cudaDeviceSynchronize());
	}
