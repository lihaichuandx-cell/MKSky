...
std::tuple<int, double, std::vector<MyDataPoint>> run_myal_algorithm(std::vector<MyDataPoint>& h_points, const AlgorithmConfig& config) {
	...
	return std::make_tuple(final_skyline_count, std::chrono::duration<double>(end - beg).count() * 1000.0, h_skyline);
}
