...
constexpr int TOPK_WITNESS_COUNT = 8;
constexpr int TOPK_WITNESS_BUILD_THRESHOLD = 1000000;
__device__ const unsigned short* g_topk_witness_offsets;
__device__ const float* g_topk_witness_thresholds;
__device__ const unsigned int* g_topk_witness_codes;
...
__global__ void build_topk_dimension_witnesses(
    const float* packed, int dim, const int* chunk_offsets,
    const int* chunk_counts, int chunk_count,
    unsigned short* witness_offsets, float* witness_thresholds) {
    const int task = blockIdx.x * blockDim.x + threadIdx.x;
    const int task_count = chunk_count * dim;
    if (task >= task_count) return;
    const int chunk = task / dim;
    const int d = task - chunk * dim;
    const int start = chunk_offsets[chunk];
    const int count = chunk_counts[chunk];
    float best_values[TOPK_WITNESS_COUNT];
    unsigned short best_offsets[TOPK_WITNESS_COUNT];
#pragma unroll
    for (int k = 0; k < TOPK_WITNESS_COUNT; ++k) {
        best_values[k] = FLT_MAX;
        best_offsets[k] = 0;
    }
    for (int local = 0; local < count; ++local) {
        int worst_slot = 0;
#pragma unroll
        for (int k = 1; k < TOPK_WITNESS_COUNT; ++k) {
            if (best_values[k] > best_values[worst_slot]) worst_slot = k;
        }
        const float value = packed[(start + local) * dim + d];
        if (value < best_values[worst_slot]) {
            best_values[worst_slot] = value;
            best_offsets[worst_slot] = static_cast<unsigned short>(local);
        }
    }
    float threshold = -FLT_MAX;
#pragma unroll
    for (int k = 0; k < TOPK_WITNESS_COUNT; ++k) {
        threshold = fmaxf(threshold, best_values[k]);
        witness_offsets[(task * TOPK_WITNESS_COUNT) + k] = best_offsets[k];
    }
    witness_thresholds[task] = threshold;
}

__global__ void build_topk_witness_codes(
    const float* packed, int dim, const int* chunk_offsets,
    const int* chunk_counts, int chunk_count,
    const unsigned short* witness_offsets,
    const float* minimums, const float* maximums,
    unsigned int* witness_codes) {
    const int item = blockIdx.x * blockDim.x + threadIdx.x;
    const int task_count = chunk_count * dim;
    const int item_count = task_count * dim;
    if (item >= item_count) return;
    const int task = item / dim;
    const int value_dimension = item - task * dim;
    const int chunk = task / dim;
    const int start = chunk_offsets[chunk];
    const int count = chunk_counts[chunk];
    const int witness_count = min(count, TOPK_WITNESS_COUNT);
    unsigned int packed_codes = 0U;
#pragma unroll
    for (int k = 0; k < TOPK_WITNESS_COUNT; ++k) {
        unsigned int code = 0U;
        if (k < witness_count) {
            const int candidate = start + static_cast<int>(
                witness_offsets[task * TOPK_WITNESS_COUNT + k]);
            code = micro_quantize(packed[candidate * dim + value_dimension],
                                  minimums[value_dimension],
                                  maximums[value_dimension]) >> 5;
        }
        packed_codes |= code << (k * 4);
    }
    witness_codes[item] = packed_codes;
}
...
#pragma unroll
    ...
    // Endpoint-XOR chunks have already undergone an exact all-pairs local skyline.
    // Fixed-partition ablations still execute the prefix check used by their SFS path.
...
#pragma unroll
    ...
    const int own_chunk = candidate_chunk_ids[index];
    int witness_dimension = 0;
    if (USE_TOPK_WITNESS) {
        unsigned int smallest_code = packed_point_codes[0] & 0xffU;
#pragma unroll
        for (int d = 1; d < DIM; ++d) {
            const unsigned int code =
                (packed_point_codes[d / 4] >> ((d % 4) * 8)) & 0xffU;
            if (code < smallest_code) {
                smallest_code = code;
                witness_dimension = d;
            }
        }
...
#pragma unroll
        ...
        const int start = chunk_offsets[chunk];
        if (USE_TOPK_WITNESS) {
            const int witness_base =
                (chunk * DIM + witness_dimension) * TOPK_WITNESS_COUNT;
            const float threshold =
                g_topk_witness_thresholds[chunk * DIM + witness_dimension];
            const float tolerance =
                EPS * fmaxf(1.0f, fabsf(point[witness_dimension]));
            if (threshold > point[witness_dimension] + tolerance) {
                const int witness_count = min(count, TOPK_WITNESS_COUNT);
                unsigned int witness_mask = MICRO_POINT_GUARDS;
#pragma unroll
                for (int d = 0; d < DIM; ++d) {
                    const unsigned int target_code =
                        (packed_point_codes[d / 4] >> ((d % 4) * 8)) & 0xffU;
                    ...
                    const unsigned int lane_result =
                        ((target_word | MICRO_POINT_GUARDS) -
                         g_topk_witness_codes[
                             (chunk * DIM + witness_dimension) * DIM + d]) &
                        MICRO_POINT_GUARDS;
                    witness_mask &= lane_result;
                    if (witness_mask == 0U) break;
                }
                for (int k = 0; k < witness_count; ++k) {
                    if (((witness_mask >> (k * 4)) & 0x8U) == 0U) continue;
                    const int candidate = start +
                        static_cast<int>(g_topk_witness_offsets[witness_base + k]);
                    if (exact_point_checks) atomicAdd(exact_point_checks, 1ULL);
                    if (point_dominates<DIM>(packed + candidate * DIM, point)) {
                        alive = false;
                        dominated_in_chunk = true;
                        break;
                    }
                }
                if (false_triggers && !dominated_in_chunk) {
                    atomicAdd(false_triggers, 1ULL);
                }
                continue;
            }
        }
...
struct ChunkVerifyLauncher {
    ...
    const unsigned int* micro_point_codes;
    const unsigned int* compatibility_masks;
    const unsigned short* topk_witness_offsets;
    const float* topk_witness_thresholds;
    const unsigned int* topk_witness_codes;
    int compatibility_words_per_row;
    int bits_per_dim;
    bool check_equal_tail;
    bool check_own_prefix;
    ...
    void launch_selected_mode() const {
        if (compatibility_masks) {
            if (topk_witness_offsets) {
                launch_compatible<DIM, PROJECTION_MODE, USE_POINT_MASK, true>();
            } else {
                launch_compatible<DIM, PROJECTION_MODE, USE_POINT_MASK, false>();
            }
        } else {
...
AlgorithmResult run_paper_variant(const std::vector<MyDataPoint>& points,
    ...
    unsigned short* topk_witness_offsets = nullptr;
    float* topk_witness_thresholds = nullptr;
    unsigned int* topk_witness_codes = nullptr;
    ...
    if (use_chunk_layout) {
        ...
        use_projection_bound = config.enable_aux_summaries &&
            ((ENABLE_PAIR_SUMMARY &&
             ...
             variant == PaperVariant::GridMkdProjectionScan ||
             variant == PaperVariant::GridMkdProjectionCycle) &&
            candidate_count >= PROJECTION_BUILD_THRESHOLD;
        ...
        if (use_chunk_skipping) {
            ...
            if (config.enable_aux_summaries &&
                variant == PaperVariant::GridMkdProjectionBound &&
                candidate_count >= PROJECTION_BUILD_THRESHOLD &&
                chunks.count >= COMPATIBILITY_MIN_CHUNKS) {
                ...
                if (compatibility_mask_bytes <= MAX_COMPATIBILITY_MASK_BYTES) {
            ...
            if (config.enable_aux_summaries &&
                variant == PaperVariant::GridMkdProjectionBound &&
                candidate_count >= PROJECTION_BUILD_THRESHOLD) {
                if (config.enable_topk_witness &&
                    dim >= MICRO_POINT_MASK_MIN_DIM &&
                    candidate_count >= TOPK_WITNESS_BUILD_THRESHOLD) {
                    const std::size_t witness_task_count =
                        static_cast<std::size_t>(chunks.count) * dim;
                    CHECK_CUDA_ERROR(cudaMalloc(&topk_witness_offsets,
                        witness_task_count * TOPK_WITNESS_COUNT *
                        sizeof(unsigned short)));
                    CHECK_CUDA_ERROR(cudaMalloc(&topk_witness_thresholds,
                        witness_task_count * sizeof(float)));
                    CHECK_CUDA_ERROR(cudaMalloc(&topk_witness_codes,
                        witness_task_count * dim * sizeof(unsigned int)));
                    build_topk_dimension_witnesses<<<
                        static_cast<unsigned int>((witness_task_count +
                            BLOCK_SIZE - 1) / BLOCK_SIZE), BLOCK_SIZE>>>(
                            packed, dim, chunk_offsets, chunk_counts,
                            chunks.count, topk_witness_offsets,
                            topk_witness_thresholds);
                    build_topk_witness_codes<<<
                        static_cast<unsigned int>((witness_task_count * dim +
                            BLOCK_SIZE - 1) / BLOCK_SIZE), BLOCK_SIZE>>>(
                            packed, dim, chunk_offsets, chunk_counts,
                            chunks.count, topk_witness_offsets, minimums,
                            maximums, topk_witness_codes);
                    CHECK_CUDA_ERROR(cudaMemcpyToSymbol(
                        g_topk_witness_offsets, &topk_witness_offsets,
                        sizeof(topk_witness_offsets)));
                    CHECK_CUDA_ERROR(cudaMemcpyToSymbol(
                        g_topk_witness_thresholds, &topk_witness_thresholds,
                        sizeof(topk_witness_thresholds)));
                    CHECK_CUDA_ERROR(cudaMemcpyToSymbol(
                        g_topk_witness_codes, &topk_witness_codes,
                        sizeof(topk_witness_codes)));
                    CHECK_CUDA_ERROR(cudaGetLastError());
                }
    ...
    if (use_chunk_skipping) {
        launch_by_dimension(dim, ChunkVerifyLauncher{
            ...
            compatibility_masks, topk_witness_offsets,
            topk_witness_thresholds, topk_witness_codes,
            compatibility_words_per_row,
            dim >= SIGNATURE_MIN_DIM ? bits_per_dim : 0,
    ...
    if (topk_witness_offsets) {
        const std::size_t witness_task_count =
            static_cast<std::size_t>(chunks.count) * dim;
        approximate_bytes += witness_task_count *
            (TOPK_WITNESS_COUNT * sizeof(unsigned short) + sizeof(float) +
    ...
    cudaFree(topk_witness_offsets);
    cudaFree(topk_witness_thresholds);
    cudaFree(topk_witness_codes);
...
AlgorithmResult run_prefix_backend_mksky(
    const std::vector<MyDataPoint>& points,
    const AlgorithmConfig& config);
AlgorithmResult run_prefix_backend_projection_bound(
    const std::vector<MyDataPoint>& points,
    const AlgorithmConfig& config);

AlgorithmResult run_paper_mksky(const std::vector<MyDataPoint>& points,
                                const AlgorithmConfig& config) {
    if (config.dim < MICRO_POINT_MASK_MIN_DIM) {
        return run_prefix_backend_mksky(points, config);
    }
...
AlgorithmResult run_paper_ablation_a(const std::vector<MyDataPoint>& points,
                                     const AlgorithmConfig& config) {
    AlgorithmConfig ablation = config;
    ablation.enable_aux_summaries = false;
    ablation.enable_topk_witness = false;
    AlgorithmResult result = run_projection_bound_mksky(points, ablation);
    result.name = "MKSky-w/o-aux-summary";
    result.provenance =
        "Controlled ablation: disable auxiliary summaries only";
    return result;
}
...
AlgorithmResult run_paper_ablation_b(const std::vector<MyDataPoint>& points,
                                     const AlgorithmConfig& config) {
    AlgorithmConfig ablation = config;
    ablation.enable_block_skipping = false;
    ablation.enable_topk_witness = false;
    AlgorithmResult result = run_projection_bound_mksky(points, ablation);
    result.name = "MKSky-w/o-block-skip";
    result.provenance =
        "Controlled ablation: disable block-level skipping only";
    return result;
}
...
AlgorithmResult run_paper_ablation_c(const std::vector<MyDataPoint>& points,
                                     const AlgorithmConfig& config) {
    AlgorithmConfig ablation = config;
    ablation.enable_mkd_partition = false;
    ablation.enable_topk_witness = false;
    AlgorithmResult result = run_projection_bound_mksky(points, ablation);
    result.name = "MKSky-w/o-MKD";
    result.provenance =
        "Controlled ablation: replace endpoint-XOR MKD with fixed chunks only";
    return result;
}
...
AlgorithmResult run_paper_ablation_no_candidate_reduction(
    ...
    AlgorithmConfig ablation = config;
    ablation.enable_candidate_reduction = false;
    ablation.enable_topk_witness = false;
    AlgorithmResult result = run_projection_bound_mksky(points, ablation);
    result.name = "MKSky-w/o-candidate-reduction";
    result.provenance =
        "Controlled ablation: retain all input points before Morton ordering";
    return result;
}
...
AlgorithmResult run_paper_ablation_no_local_prefix(
    const std::vector<MyDataPoint>& points, const AlgorithmConfig& config) {
    AlgorithmConfig ablation = config;
    ablation.enable_local_prefix_filtering = false;
    ablation.enable_topk_witness = false;
    AlgorithmResult result = run_projection_bound_mksky(points, ablation);
    result.name = "MKSky-w/o-local-prefix";
    result.provenance =
        "Controlled ablation: keep MKD chunks but bypass intra-chunk prefix filtering";
    return result;
}
...
AlgorithmResult run_projection_bound_mksky(const std::vector<MyDataPoint>& points,
                                           const AlgorithmConfig& config) {
    AlgorithmConfig submission_config = config;
    submission_config.enable_topk_witness = false;
    AlgorithmResult result;
    if (submission_config.dim < MICRO_POINT_MASK_MIN_DIM) {
        result = run_prefix_backend_projection_bound(points, submission_config);
    } else {
        result = run_paper_variant(points, submission_config,
                                   PaperVariant::GridMkdProjectionBound);
    }
    result.name = "MKSky";
    result.provenance =
        "Submission MKSky: grid/pivot pruning, Morton ordering, endpoint-XOR "
        "MKD, prefix-local filtering, safe block summaries, and exact verification";
    return result;
}
